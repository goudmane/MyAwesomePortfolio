...
const currentLang = computed({
    get: () => locale.value,  // Return the current locale
    set: (value) => {
        locale.value = value;  // Update the locale when changed
    },
});
...
// Function to scroll to top and return a promise when the scroll ends
...
body {
    ...
    // Element centered for each browser
    display: -webkit-box;
    display: -moz-box;
    display: -ms-flexbox;
    display: -webkit-flex;
...
.slider {
    ...
    &:before {
        ...
        vertical-align: middle; /* Align the flag with text */
        left: 0;
        bottom: 0;
        -webkit-transition: 0.4s;
        transition: 0.4s;
    ...
    &.fr:before{
        box-shadow: -2px 0px 13px 0px $gold;
